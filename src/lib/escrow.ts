...
import { sendContractTx, sendMulticall, encodeCall, type MulticallCall } from './wallet';
...
// Batch void multiple sessions in a single transaction via Multicall3
export async function batchVoid(
  networkId: string,
  paymentInfos: PaymentInfo[]
): Promise<BatchVoidResult> {
  ...
  try {
    const network = await getNetwork(networkId);
    if (!network) {
      return { success: false, error: `Network ${networkId} not found`, sessionCount: 0 };
    }

    // Build multicall calls for each void
    const calls: MulticallCall[] = paymentInfos.map((paymentInfo) => ({
      target: network.escrow_contract as Address,
      allowFailure: true, // Allow individual failures
      callData: encodeCall(ESCROW_ABI, 'void', [toPaymentInfoTuple(paymentInfo)]),
    }));

    const result = await sendMulticall(networkId, calls);

    return {
      success: result.success,
      txHash: result.txHash,
      error: result.error,
      sessionCount: paymentInfos.length,
    };
  } catch (err) {
    console.error('Batch void error:', err);
    const error = err instanceof Error ? err.message : 'Unknown error';
    return { success: false, error, sessionCount: 0 };
  }
}

interface CaptureAndVoidItem {
  paymentInfo: PaymentInfo;
  captureAmount: bigint; // Amount to capture (pending)
}

interface BatchCaptureAndVoidResult {
  success: boolean;
  txHash?: string;
  error?: string;
  captureCount: number;
...
// Batch capture and void multiple sessions in a single transaction via Multicall3
// This is used for reclaim-all: first capture pending amounts, then void remaining
export async function batchCaptureAndVoid(
  networkId: string,
  items: CaptureAndVoidItem[],
  feeBps: number = 0,
  feeReceiver: string = ZERO_ADDRESS
): Promise<BatchCaptureAndVoidResult> {
  if (items.length === 0) {
    return { success: true, captureCount: 0, voidCount: 0, totalCaptured: 0n };
  }
  ...
  try {
    ...
    // Build calls: captures first, then voids
    for (const item of items) {
      // Add capture call if there's pending amount
      if (item.captureAmount > 0n) {
        calls.push({
          target: network.escrow_contract as Address,
          allowFailure: true,
          callData: encodeCall(ESCROW_ABI, 'capture', [
            toPaymentInfoTuple(item.paymentInfo),
            item.captureAmount,
            feeBps,
            feeReceiver as Address,
          ]),
        ...
        captureCount++;
        totalCaptured += item.captureAmount;
      }
      ...
      calls.push({
        target: network.escrow_contract as Address,
        allowFailure: true,
        callData: encodeCall(ESCROW_ABI, 'void', [toPaymentInfoTuple(item.paymentInfo)]),
      });
    }
    ...
    return {
      ...
      error: result.error,
      captureCount,
      voidCount: items.length,
      totalCaptured,
    };
  } catch (err) {
    console.error('Batch capture+void error:', err);
    const error = err instanceof Error ? err.message : 'Unknown error';
    return { success: false, error, captureCount: 0, voidCount: 0, totalCaptured: 0n };
  }
}
