...
// GET /api/sessions/[id] - Get session detail for operator (incoming sessions)
export async function GET(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    // Verify session belongs to this operator (created via their API key)
    if (session.user_id !== auth.userId) {
      log.warn('Session access denied', { sessionId, userId: auth.userId });
      return NextResponse.json(
        { error: 'Session not found' },
        { status: 404, headers: rateLimitHeaders(rateLimit) }
      );
    ...
    log.debug('Operator session detail fetched', { sessionId, userId: auth.userId });
    ...
    log.error('Get operator session error', err instanceof Error ? err : new Error(String(err)));
